...
describe("post detail reading experience", () => {
  ...
  it("Given a long post When rendering detail Then it shows prose, linked taxonomy, heading anchors, and TOC links", async () => {
    ...
    expect(markup).toContain('id="heading-toc를-오른쪽에-두는-방식"')
    ...
    expect(markup).toContain('href="#heading-toc를-오른쪽에-두는-방식"')
    expect(markup).toContain('href="#heading-추천-조합"')
    expect(markup).not.toContain('href="#heading-글-상세-화면에서-읽기-도구를-배치하는-방법"')
