      ...
      <ol className="mt-4 space-y-2">
        {items.map((item) => (
          <li className={item.level === 3 ? "pl-4" : undefined} key={item.id}>
            <a
              className="block rounded-sm py-0.5 text-muted-foreground underline-offset-4 transition-colors duration-150 hover:text-foreground hover:underline focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring"
              href={`#${item.id}`}
            >
              {item.text}
            </a>
